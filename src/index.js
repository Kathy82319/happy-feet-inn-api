...
function handleCorsPreflight() {
    return new Response(null, {
        headers: {
            ...
            "Access-Control-Allow-Headers": "Content-Type, X-LINE-CHANNEL-ID, X-LINE-AUTHORIZATION-NONCE, X-LINE-AUTHORIZATION",
        },
    });
}
...
async function handleCreatePayment(request, env, LINE_PAY_API_URL) {
    const { bookingId, rowNumber } = await request.json();
...
async function writeBookingToSheet(env, booking) {
    ...
    if (!readResponse.ok) throw new Error("Failed to read booking count from Google Sheet");
    const sheetData = await readResponse.json();
    const existingRows = sheetData.values ? sheetData.values.length : 0;
    const newRowNumber = existingRows + 1;
...
async function sendPaymentSuccessMessage(env, bookingDetails) {
    ...
    const flexMessage = {
        ...
        hero: {
            type: "image", url: room.imageUrl || 'https://placehold.co/1024x512?text=Payment+Confirmed',
...
async function getAvailabilityForRoom(env, roomId, startDateStr, endDateStr) {
    ...
    return {
        roomId, startDate: startDateStr, endDate: endDateStr, availableCount: finalCount,
    };
}
